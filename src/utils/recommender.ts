...
export function generateAlternatives(originalName: string): string[] {
	...
	const commonSuffixes = ['js', 'ts', 'api', 'core', 'lib', 'kit', 'pro', 'plus'];
	const commonPrefixes = ['my', 'super', 'easy', 'pro', 'ultra'];
	const contextWords = ['utils', 'helper', 'toolkit', 'framework', 'module'];

	// Base + suffixes
	commonSuffixes.forEach(suffix => alternatives.add(`${normalized}-${suffix}`));

	// Base + prefixes
	commonPrefixes.forEach(prefix => alternatives.add(`${prefix}-${normalized}`));

	// Base + context words (as suffixes)
	contextWords.forEach(word => alternatives.add(`${normalized}-${word}`));
	...
	if (synonyms) {
		words.forEach((word: string, i: number) => {
			...
			const syns: string[] = synonymDict ? (Object.values(synonymDict).flat() as string[]) : [];
			syns.slice(0, 3).forEach((syn: string) => { // Limit to top 3 synonyms per word
